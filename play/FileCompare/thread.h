 ...
 *
 * 默认的拷贝构造函数只有类没有体现出bitwise copy semantics是编译器必须合成
 * 一下四种情况编译器没有体现出bitwose copy semantics
 ...
 *
  *QThread对象存活于该对象被创建的线程中，而并非Qhread::run()函数被调用时所在的线程
  ...
  *Qt的默认连接方式,如果信号的发出与接收信号的对象同属一个线程，那个工作方式与直连方式相同
  *否则工作方式与队列方式相同。
  ...
  *QThread对象的线程，QThread管理的线程，就是run()函数启动的线程，也就是新线程。QThread对象衣服在主线程中，QThread
