...
{
    int count = 0;
    if (!line->isEmpty() && !keywords.isEmpty())
    {
        for (int i=0; i<line->length(); i++)
        {
            if (!QString::compare(line->at(i), "+", Qt::CaseInsensitive))
            {
                count++;
                if (4 == count)
                {
                    if (linenum == 20)
                        line->replace(i+4, keywords.length() * 2, keywords);
                    else if (linenum == 21)
                        line->replace(i-4, keywords.length() * 2, keywords);
...
QString DrawHeart(const QString& keywords, LOVE type,  const QDateTime& datetime, bool isReplace, const QStringList& wordlist)
{
    ...
    QStringList heartlist = heart.split("\n", QString::SkipEmptyParts);
    heartlist.removeAll(heartlist.at(0));

    if (isReplace)
    {
        ReplaceHeart(const_cast<QString *>(&heartlist.at(21)), keywords, 21);
        ReplaceHeart(const_cast<QString *>(&heartlist.at(20)), gbk2string("中秋遇上国庆"), 20);
    }
    heart = heartlist.join('\n');

    if (type == SUSU_GUAGUA)
    {
        heart = heart % datetime.toString(QStringLiteral("\n\t\t\t\t\t\t\t\t\t\tyyyy年MM月dd日 hh点mm分ss秒")) % QStringLiteral("我和瓜瓜在一起了\n") %
                QStringLiteral("\t\t\t\t\t\t\t\t\t\t\t我们在一起") % CalculateDateTime(datetime) % QStringLiteral("了");
    }
    else if (type == OTHERS && wordlist.count() > 0)
    {
        for(int i=0; i<wordlist.count(); i++)
        {
            heart = heart % QObject::tr("\n\t\t\t\t\t\t\t\t\t\t%1").arg(wordlist.at(i));
        }
    }
    return heart;
}
...
QStringList checkString(const QString &str)
{
    ...
    {
        if(QString(*iter).toInt() >= QString(*(iter++)).toInt())
        {
            qDebug().noquote()<<u8"字符串不符合要求，请检查输入的字符串是否是递增的";
            flag = false;
            return QStringList();
        }
    }
...
QString dealString(const QStringList &strList)
{
    if(strList.count() < 1)
    {
        qDebug().noquote()<<u8"字符串数组为空";
        return "";
    }
    QString retStr = QString(strList.at(0)).trimmed();
    bool flag = false;
    for(int i=0; i < strList.count()-1; i++)
    {
        if(QString(strList.at(i)).trimmed().toInt() + 1 == QString(strList.at(i+1)).trimmed().toInt())
        {
            if(i != 0 && QString(strList.at(i-1)).trimmed().toInt() + 1 != QString(strList.at(i)).trimmed().toInt())
                retStr = retStr % strList.at(i);
            flag = true;
        }
        else
        {
            if (flag)
            {
                retStr = retStr % QStringLiteral("-") % QString(strList.at(i)).trimmed() % QStringLiteral(",");
                flag = false;
            }
            else
            {
                if(i != 0)
                    retStr = retStr % QString(strList.at(i)).trimmed() % ",";
                else
                    retStr = retStr % QStringLiteral(",");
            }
        }
        if(i == strList.count()-2)
        {
            if(flag)
            {
                retStr = retStr % QStringLiteral("-") % QString(strList.at(i+1)).trimmed();
            }
            else
            {
                retStr = retStr % QString(strList.at(i+1)).trimmed();
            }
        }
    }
    return retStr;
...
void RemoveFileThan30Days(const QString &dirpath, int size)
{
    QDir dir(dirpath);
    if (dir.exists())
    {
        ...
        for (QFileInfo file : filelist)
        {
            if (file.isFile())
            {
                ...
                if (file.lastModified().daysTo(currentDateTime)>30 || file.size() > size * 10 * 1024 * 1024)
                {
...
QSqlDatabase CreateDB(QString dbName)
{
    QSqlDatabase db;
    ...
    if(QFile::exists(dbName))
    {
        ...
        OUT << QStringLiteral("获取数据库：LoveDiary.db：内容");
    }
    else
    {
        OUT << QObject::tr("数据库文件%1不存在").arg(dbName);
    }
    return db;
}
...
void AppLog(QtMsgType type, const QMessageLogContext & context, const QString &msg)
{
    ...
    RemoveFileThan30Days(dirstr);
    QString fileName(dirstr);
    fileName = fileName % QDateTime::currentDateTime().toString(QStringLiteral("yyyy_MM_dd")) % QString(QStringLiteral("_%1.log")).arg(appName);
    QFileInfo fileInfo(context.file);
    QString fileStr(fileInfo.fileName());
    int line = context.line;
    QString dateStr = QDateTime::currentDateTime().toString(QStringLiteral("yyyy_MM_dd hh::mm::ss zzz"));
    QString logdata = QString::null;

    QStringList lovekeylist;
    if (!appName.isEmpty() && appName == "LoveDiary")
    {
        lovekeylist << "love&debug" << "love&warning" << "love&critical" << "love&fatal" << "love&info";
    }
    else
    {
        lovekeylist << "debug" << "warning" << "critical" << "fatal" << "info";
    }

    switch(type)
    {
    case QtDebugMsg:
        logdata = logdata % QString(u8"[%5~%1 %2-%3]:%4").arg(dateStr).arg(fileStr).arg(line).arg(msg).arg(lovekeylist.at(0));
        break;
    case QtWarningMsg:
        logdata = logdata % QString(u8"[%5~%1 %2-%3]:%4").arg(dateStr).arg(fileStr).arg(line).arg(msg).arg(lovekeylist.at(1));
        break;
    case QtCriticalMsg:
        logdata = logdata % QString(u8"[%5~%1 %2-%3]:%4").arg(dateStr).arg(fileStr).arg(line).arg(msg).arg(lovekeylist.at(2));
        break;
    case QtFatalMsg:
        logdata = logdata % QString(u8"[%5~%1 %2-%3]:%4").arg(dateStr).arg(fileStr).arg(line).arg(msg).arg(lovekeylist.at(3));
        break;
    case QtInfoMsg:
        logdata = logdata % QString(u8"[%5~%1 %2-%3]:%4").arg(dateStr).arg(fileStr).arg(line).arg(msg).arg(lovekeylist.at(4));
        break;
    }
    logdata += "\n";
    QFile file(fileName);
...
void Sleep(int mesc)
{
    ...
    {
#if (QT_VERSION < QT_VERSION_CHECK(5, 0, 0))
        QTime endTime = QTime::currentTime().addMSecs(mesc);
...
void CalculateTime(const qint64 lavesecs, qint64 &hours, qint64 &minutes, qint64 &seconds)
{
    ...
    minutes = (lavesecs - hours * 60 * 60) / 60;
...
QString CalculateDateTime(const QDateTime & datetime)
{
    ...
    qint64 secs = datetime.secsTo(currentDateTime);
    ...
    return  QString::number(days) % QStringLiteral("天") % QString::number(hours) % QStringLiteral("时") % \
            QString::number(minutes) % QStringLiteral("分") % QString::number(seconds) % QStringLiteral("秒");
}
...
void RunWithSystem(const QString &strName, const QString &strPath, bool autoRun)
{
#ifdef Q_OS_WIN
...
{
    QRegExp regexp(QStringLiteral("((2[0-4]\\d|25[0-5]|[01]?\\d\\d?)\\.){3}(2[0-4]\\d|25[0-5]|[01]?\\d\\d?)"));
    return regexp.exactMatch(ip);
}

bool IsMac(const QString &mac)
{
    QRegExp regexp(QStringLiteral("^[A-F0-9]{2}(-[A-F0-9]{2}}){5}$"));
...
QString StrHexToStrBin(const QString &strHex)
{
    ...
    if (len < 8)
    {
        ...
        {
            bin = QStringLiteral("0") % bin;
        }
    }
    return bin;
}

QString DecimalToStrBin1Byte(int decimal)
{
    QString hex = QString::number(decimal, 16);

    bool ok;
    int bin = hex.toInt(&ok, 16);

    bin = bin & 0xFF;

    QString binstr = QString::number(bin, 2);

    if (binstr.length() < 8)
    {
        for (int i=0; i<8-binstr.length(); i++)
        {
            binstr = QStringLiteral("0") + binstr;
        }
    }
...
QString DecimalToStrBin2Byte(int decimal)
{
    QString hexstr = QString::number(decimal, 16);
    ...
    QString binstr = QString::number(hex, 2);
...
int ByteToInt(const QByteArray &data)
{
...
int ByteToIntRec(const QByteArray &data)
{
...
quint32 ByteToUInt(const QByteArray &data)
{
...
quint32 ByteToUIntRec(const QByteArray &data)
{
...
int ByteToUShort(const QByteArray &data)
{
...
int ByteToUShortRec(const QByteArray &data)
{
    int i = data.at(0) & 0x000000FF;
    i |= ((data.at(1) << 8) & 0x0000FF00);
...
uchar GetOrCode(const QByteArray &data)
{
    int len = data.length();
    uchar result = 0;
...
uchar GetCheckCode(const QByteArray &data)
{
    int len = data.length();
    uchar temp = 0;
...
//逆序CRC计算
quint16 GetRevCrc_16(quint8 *data, int len, quint16 init, const quint16 *table)
{
    quint16 cRc_16 = init;
    quint8 temp;
...
//正序CRC计算
quint16 GetCrc_16(quint8 *data, int len, quint16 init, const quint16 *table)
{
    quint16 cRc_16 = init;
    quint8 temp;
...
quint16 GetModbus16(quint8 *data, int len)
{
...
bool IpLive(const QString &ip, int port, int timeout)
{
    //局部的事件循环,不卡主界面
...
QString GetJianPin(const QString &src)
{
...
void getConfig(const QString &content, qint32 &day, qint32 &hour, qint32 &minute, bool &flag)
{
    ...
    for(int i=0; i<alllist.count(); i++)
    {
        ...
        if (line.contains('='))
        {
            ...
            if (line.startsWith("wfrstartui_day"))
            {
                configlist = line.split('=', QString::SkipEmptyParts);
                if (configlist.count() == 2 && !QString(configlist.at(1)).isEmpty())
                {
                    day = QString(configlist.at(1)).trimmed().toInt();
                    if (day > 6)
                    {
                        day %= 7;
                    }
                    ...
                    {
                        day = Qt::Sunday - QDateTime::currentDateTime().date().dayOfWeek();;
                    }
                }
            }
            else if (line.startsWith("wfrstartui_hour"))
            {
                configlist = line.split('=', QString::SkipEmptyParts);
                if (configlist.count() == 2 && !QString(configlist.at(1)).isEmpty())
                {
                    hour = QString(configlist.at(1)).trimmed().toInt();
                    if (hour > 23)
                    {
                        hour %= 24;
                    }
            ...
            else if (line.startsWith("wfrstartui_minute"))
            {
                configlist = line.split('=', QString::SkipEmptyParts);
                if (configlist.count() == 2 && !QString(configlist.at(1)).isEmpty())
                {
                    minute = QString(configlist.at(1)).trimmed().toInt();
                    if (minute > 59)
                    {
                        minute %= 60;
                    }
            ...
            else if (line.startsWith("wfrstartui_base_now"))
            {
                configlist = line.split('=', QString::SkipEmptyParts);
                if (configlist.count() == 2 && !QString(configlist.at(1)).isEmpty())
                {
                    if (QString(configlist.at(1)).trimmed().toInt())
                    {
                        flag = true;
                    }
                    else
